...
class Solution {
public:
    bool isSubsequence(string s, string t) { // A function that takes two strings as input and returns a boolean value
        int m = s.size(); // Size of string s
        int n = t.size(); // Size of string t
        int i = 0, j = 0; // Initializing two counters/Pointes to 0
        while(i < m && j < n) { // While i and j are less than the size of s and t respectively
            if(s[i] == t[j]) { // If the character at index i of s is the same as the character at index j of t
                i++; // Increment i by 1
            }
            j++; // Increment j by 1
        }
